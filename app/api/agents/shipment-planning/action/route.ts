...
export async function POST(req: Request) {
  ...
  await createWorkflowAgentAction({
    agent_id: "shipment-planning",
    subject_type: "order",
    subject_id: body.capture_id,
    recommended_decision: String(body.proposal.plan_status ?? body.final_decision),
    final_decision: body.final_decision,
    summary: String(body.proposal.summary ?? "Shipment plan reviewed."),
    payload_json: JSON.stringify(body.proposal),
    actor: "shipment-planning-review",
  });

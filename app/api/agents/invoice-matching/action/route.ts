...
export async function POST(req: Request) {
  ...
  await createWorkflowAgentAction({
    agent_id: "invoice-matching",
    subject_type: "invoice",
    subject_id: body.invoice_id,
    recommended_decision: String(body.proposal.match_status ?? body.final_decision),
    final_decision: body.final_decision,
    summary: String(body.proposal.summary ?? "Invoice match recommendation reviewed."),
    payload_json: JSON.stringify(body.proposal),
    actor: "invoice-matching-review",
  });
